...
robjects_out <- paste0(data_path, "/msopena/02_OneK1K_Age/robjects/07_DEA_SexAge/")
...
dreamlet.func <- function( ge_dge, sex ){
  ### Defining the DEA formulas ###
  print('Defining the DEA formulas...')
  form <- as.formula(~Age+(1|date))
  ...
  out <- list(dea = res.dl,
              topTable = toptable)
  out_fn <- paste0(robjects_out,cell_type,'_', sex, '_unbalanced_deaTopTable.rds')
...
# 2. dreamlet
dreamlet.func(pb_F, "F")
dreamlet.func(pb_M, "M")
